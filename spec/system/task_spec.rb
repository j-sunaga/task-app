  ...
  describe 'タスク一覧画面' do
    ...
    context '複数のタスクを作成した場合' do
      ...
      it 'タスクが作成日時の降順に並んでいる' do
        act_as user do
          ...
          expect(task_list[1]).to have_content task.name.to_s
        end
      end
    end
    ...
    context '検索をした場合' do
      ...
      it 'タイトルで検索できる' do
        act_as user do
          visit tasks_path
          fill_in 'name', with: tasks[0].name.to_s
          click_on 'commit'
          expect(page).to have_content tasks[0].name.to_s
          expect(page).to_not have_content tasks[1].name.to_s
        end
      end
      it 'ステータスで検索できる' do
        act_as user do
  ...
  describe 'ラベル管理機能' do
    ...
    context 'ラベルを登録した場合' do
      it '作成済みのラベルが一覧画面に表示される' do
        act_as user do
          visit labels_path
          expect(page).to have_content label.name.to_s
        end
      end
    end
    context 'タスクにラベルを紐付けた場合' do
      ...
      it '該当タスクが検索できる' do
        act_as user do
          visit tasks_path
          select tasks[0].labels.first.name.to_s, from: 'label'
          click_on 'commit'
          expect(page).to have_content tasks[0].name.to_s
          expect(page).to have_content tasks[0].labels.first.name.to_s
        end
      end
    end
  end
